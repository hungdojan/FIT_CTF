...
All notable changes to FIT-CTF are listed here. The same history is maintained in
`CHANGELOG.md` at the repository root.

The format follows `Keep a Changelog <https://keepachangelog.com/>`__ and
...
- Local MongoDB tasks (``inv db-start``, ``db-stop``, ``db-shell``) use Podman instead of
  Docker.
- ``PodmanClient.compose_ps`` uses ``podman-compose ps -q`` to avoid false positives when
...
- `Unreleased changes <https://github.com/hungdojan/fit-ctf/compare/v1.0.0...HEAD>`__
- `v1.0.0 release <https://github.com/hungdojan/fit-ctf/releases/tag/v1.0.0>`__
